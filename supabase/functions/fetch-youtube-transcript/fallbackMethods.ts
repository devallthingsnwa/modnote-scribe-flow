    ...
    try {
      ...
      if (captionsResponse.ok) {
        ...
        if (captionsData.items && captionsData.items.length > 0) {
          ...
          const placeholderContent = `Captions detected for "${metadata.title}" by ${metadata.author}. Content extraction requires OAuth authentication. Try the direct caption API methods instead.`;
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (xmlContent.includes('<text')) {
              // We need to get title and author from somewhere
              // Try to fetch basic info from watch page for metadata
              ...
              try {
                ...
                // Fallback to defaults if watch page fetch fails
              }
...
---
## 📝 Notes
Unable to fetch transcript. Please try a different video or check if captions are available.`,
        metadata: {
          videoId: '',
          title: 'Error',
          author: '',
