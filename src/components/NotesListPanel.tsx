...
export function NotesListPanel({
  ...
  if (isCollapsed) {
    return (
      <div className="flex flex-col h-full bg-background/50 border-r border-border">
        ...
        <div className="flex-1 flex flex-col items-center gap-3 p-3">
          ...
          {onImport && (
            <Button onClick={onImport} variant="outline" size="icon" className="h-9 w-9 hover:bg-muted">
      ...
      <div className="p-4 border-b border-border space-y-4 bg-gradient-to-b from-background to-background/80">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            {onToggleCollapse && (
              <Button variant="ghost" size="icon" onClick={onToggleCollapse} className="h-8 w-8 hover:bg-primary/10 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {onSelectModeToggle && (
              <Button 
                ...
                className="text-xs px-3 h-8 transition-all"
              >
            ...
            {onImport && !isSelectMode && (
              <Button 
                ...
                size="sm" 
                variant="outline" 
                className="text-xs px-3 h-8 hover:bg-primary/10 hover:border-primary/30 transition-all"
              >
            ...
            {!isSelectMode && (
              <Button 
                onClick={onNewNote} 
                size="sm" 
                className="text-xs px-3 h-8 bg-primary hover:bg-primary/90 shadow-sm transition-all"
              >
          ...
          <div className="p-3 space-y-2">
            {notes.map((note) => (
              <Card 
                key={note.id} 
                className={cn(
                  "p-4 cursor-pointer transition-all duration-200 border group hover:shadow-md",
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        {note.is_transcription && (
                          <Badge variant="secondary" className="text-xs px-2 h-5 bg-red-50 text-red-600 border-red-200">
                        ...
                        {note.tags.length > 0 && (
                          <div className="flex gap-1">
                            {note.tags.slice(0, 1).map((tag) => (
                              <Badge key={tag.id} variant="outline" className="text-xs px-2 h-5">
                                {tag.name}
                              </Badge>
                            ))}
                            {note.tags.length > 1 && (
