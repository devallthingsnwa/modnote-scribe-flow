  ...
  if (isCollapsed) {
    return <div className="flex flex-col h-full bg-background border-r border-border">
        ...
        <div className="flex-1 flex flex-col items-center gap-2 p-2">
          <Button onClick={onNewNote} size="icon" className="h-8 w-8">
            <Plus className="h-4 w-4" />
          </Button>
          {onImport && <Button onClick={onImport} variant="outline" size="icon" className="h-8 w-8">
  ...
  return <div className="flex flex-col h-full bg-background">
      ...
      <div className="p-3 border-b border-border space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {onToggleCollapse && <Button variant="ghost" size="icon" onClick={onToggleCollapse} className="h-6 w-6 flex-shrink-0">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            {onSelectModeToggle && <Button onClick={onSelectModeToggle} size="sm" variant={isSelectMode ? "default" : "outline"} className="text-xs px-2 h-8">
                <MoreHorizontal className="h-3 w-3" />
                <span className="hidden sm:inline ml-1">
                  {isSelectMode ? "Cancel" : "Select"}
                </span>
              </Button>}
            {onImport && !isSelectMode && <Button onClick={onImport} size="sm" variant="outline" className="text-xs px-2 h-8">
                <Upload className="h-3 w-3" />
                <span className="hidden sm:inline ml-1">Import</span>
              </Button>}
            {!isSelectMode && <Button onClick={onNewNote} size="sm" className="text-xs px-2 h-8">
        ...
        {/* Select Mode Actions */}
        {isSelectMode && <div className="flex items-center justify-between p-2 bg-muted rounded-md">
        ...
        {!isSelectMode && <div className="relative">
            ...
            <Input placeholder="Search notes..." value={searchQuery} onChange={e => onSearchChange(e.target.value)} className="pl-10" />
          </div>}
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? <div className="p-4 text-center text-muted-foreground">
            Loading notes...
          </div> : notes.length === 0 ? <div className="p-4 text-center">
            <div className="text-muted-foreground mb-2">No notes found</div>
            <Button onClick={onNewNote} variant="outline" size="sm">
              Create your first note
            </Button>
          </div> : <div className="p-2">
            {notes.map(note => <Card key={note.id} className={cn("p-3 mb-2 cursor-pointer transition-all hover:shadow-sm border", selectedNoteId === note.id && !isSelectMode ? "bg-primary/10 border-primary/30 shadow-md" : "hover:bg-muted/50 border-border", selectedNoteIds.includes(note.id) && isSelectMode ? "bg-primary/10 border-primary/30" : "")} onClick={() => onNoteSelect(note.id)}>
                <div className="flex gap-3">
                  {/* Checkbox for select mode */}
                  {isSelectMode && <div className="flex items-center pt-1">
                      <Checkbox checked={selectedNoteIds.includes(note.id)} onClick={e => e.stopPropagation()} onChange={() => onNoteSelect(note.id)} />
                    </div>}
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    {note.content && <p className="text-xs text-muted-foreground line-clamp-2">
                    ...
                    addSuffix: true
                      ...
                      <div className="flex items-center gap-1">
                        {note.is_transcription && <Badge variant="secondary" className="text-xs px-1 h-4">
                            <Video className="h-2 w-2 mr-1" />
                            Video
                          </Badge>}
                        {note.tags.length > 0 && <div className="flex gap-1">
                            {note.tags.slice(0, 1).map(tag => <Badge key={tag.id} variant="secondary" className="text-xs px-1 h-4">
                                {tag.name}
                              </Badge>)}
                            {note.tags.length > 1 && <Badge variant="secondary" className="text-xs px-1 h-4">
                                +{note.tags.length - 1}
                              </Badge>}
                          </div>}
                      </div>
