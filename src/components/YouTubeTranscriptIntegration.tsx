...
export function YouTubeTranscriptIntegration({
  ...
  const isValidTranscript = (transcript: string): boolean => {
    ...
    const errorMessages = ['You can add your own notes here', 'Transcript not available', 'No transcript available', 'Unable to fetch transcript', 'Error fetching transcript', 'could not be fetched', 'Please try again later', 'All extraction methods failed', 'captions available or may be restricted', 'technical error'];
    const hasErrorMessage = errorMessages.some(msg => trimmed.toLowerCase().includes(msg.toLowerCase()));
